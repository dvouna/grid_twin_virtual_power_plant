...
# Fallbacks allow local development/testing; production must set these via env-vars.
INFLUX_CLOUD_URL = os.getenv("INFLUX_CLOUD_URL", "http://localhost:8086")
INFLUX_CLOUD_TOKEN = os.getenv("INFLUX_CLOUD_TOKEN", "default-token-for-dev")
INFLUX_CLOUD_ORG = os.getenv("INFLUX_CLOUD_ORG", "Energy Simulation")
INFLUX_CLOUD_BUCKET = os.getenv("INFLUX_CLOUD_BUCKET", "energy")
...
class BatteryManagementAgent:
    ...
    def __init__(self, capacity_kwh: float = 10000.0, max_c_rate: float = 1.0, base_wear_cost_per_kwh: float = 0.015):
        ...
        self.total_energy_cycled_kwh = 0.0  # Cumulative absolute energy throughput (kWh)
        self.max_dispatch_kw = capacity_kwh * max_c_rate

        # A1: Initialize persistent InfluxDB client and APIs
        self.client = InfluxDBClient(url=INFLUX_CLOUD_URL, token=INFLUX_CLOUD_TOKEN, org=INFLUX_CLOUD_ORG)
        self.write_api = self.client.write_api(write_options=SYNCHRONOUS)
        self.query_api = self.client.query_api()

    def close(self) -> None:
        """
        Cleanly shut down the InfluxDB client and its associated APIs.
        """
        try:
            self.write_api.close()
            self.client.close()
            logger.info("BAM: InfluxDB client connection closed.")
        except Exception as e:
            logger.error(f"BAM: Error closing InfluxDB client ({e})")

    # ------------------------------------------------------------------
    # A1: State Persistence helpers
    def _restore_state(self) -> None:
        """
        On startup, query InfluxDB for the latest persisted SoC and total_energy_cycled_kwh
        and restore them. Falls back to safe defaults if no prior record exists.
        """
        try:
            # Fetch both persisted state fields in one query
            query = (
                f'from(bucket: "{INFLUX_CLOUD_BUCKET}")'
                f" |> range(start: -30d)"
                f' |> filter(fn: (r) => r["_measurement"] == "bam_state")'
                f' |> filter(fn: (r) => r["_field"] == "soc_kwh" or r["_field"] == "total_energy_cycled_kwh")'
                f" |> last()"
            )
            ...
            for table in tables:
                for record in table.records:
                    ...
                    fields_restored.add(field)

                    if field == "soc_kwh":
                        # Clamp to physical limits before trusting the stored value
                        self.current_soc_kwh = max(0.0, min(self.capacity_kwh, value))
                        logger.info(
                            f"BAM: Restored SoC from InfluxDB \u2192 {self.current_soc_kwh:.2f} kWh "
                            f"({self.soc_percentage * 100:.1f}%)"
                        )
                    elif field == "total_energy_cycled_kwh":
                        self.total_energy_cycled_kwh = max(0.0, value)
                        logger.info(
                            f"BAM: Restored total_energy_cycled_kwh from InfluxDB \u2192 "
                            f"{self.total_energy_cycled_kwh:.2f} kWh"
                        )

            if "soc_kwh" not in fields_restored:
                logger.warning("BAM: No prior SoC record found in InfluxDB. Starting at 50%.")
            if "total_energy_cycled_kwh" not in fields_restored:
                logger.warning("BAM: No prior lifecycle record found in InfluxDB. Starting at 0.0 kWh.")
        except Exception as e:
    ...
    def _persist_state(self) -> None:
        ...
        try:
            ...
            self.write_api.write(bucket=INFLUX_CLOUD_BUCKET, org=INFLUX_CLOUD_ORG, record=point)
        except Exception as e:
    ...
    def _write_dispatch_log(
        ...
        Args:
            soc_kwh_snapshot: SoC captured inside the lock at the exact moment of state
                              mutation, guaranteeing the logged value is consistent with
                              this specific dispatch even under concurrent load.
        """
        try:
            point = (
                ...
                .field("soc_kwh", soc_kwh_snapshot)
                .field("soc_pct", soc_kwh_snapshot / self.capacity_kwh)
            )
            self.write_api.write(bucket=INFLUX_CLOUD_BUCKET, org=INFLUX_CLOUD_ORG, record=point)
        except Exception as e:
    ...
    def efc(self) -> float:
        ...
        One full cycle is defined as a full charge followed by a full discharge
        (i.e., total throughput of 2x the battery capacity).
        """
        return self.total_energy_cycled_kwh / (2 * self.capacity_kwh)
    ...
    def _calculate_degradation_cost(self, kw_requested: float, duration_hours: float) -> float:
        ...
        Returns:
            The degradation cost in Pounds (£).
        """
    ...
    def evaluate_request(
        ...
        Args:
            ...
            expected_revenue: The total anticipated absolute profit context from financial trades (£).
                             (Note: Currently expects absolute reward for the duration, not a rate).
        ...
        with self.lock:
            ...
            # NOTE: We do NOT return early here. Using a flag instead ensures the lock is
            # released cleanly and _write_dispatch_log() is always called for audit purposes.
            ...
            if not vetoed:
                # Update official state — only for approved, non-vetoed dispatches.
                # Apply max/min clamping to prevent floating-point drift out of physical bounds.
                new_soc_kwh = self.current_soc_kwh + approved_kw * duration_hours
                self.current_soc_kwh = max(0.0, min(self.capacity_kwh, new_soc_kwh))
                # Accumulate absolute energy throughput regardless of charge/discharge direction
                self.total_energy_cycled_kwh += abs(approved_kw) * duration_hours

            # Capture SoC inside the lock so the audit log is consistent with this specific
            # dispatch, even if another thread updates state immediately after.
            soc_snapshot = self.current_soc_kwh

        # --- Outside the lock ---
        # A1: Persist new SoC after releasing the lock so InfluxDB I/O latency never
        # causes other dispatch requests to queue behind a network call.
        if not vetoed:
            self._persist_state()
        ...
        # A2: Write authoritative dispatch record to InfluxDB (always — including vetoes)
        self._write_dispatch_log(agent_name, requested_kw, approved_kw, log_status, soc_snapshot)
...
class DispatchRequest(BaseModel):
    ...
    expected_revenue_per_kwh: float = 0.0  # Total absolute reward context (£) for the duration
...
def get_state():
    return {
        ...
        "total_energy_cycled_kwh": bam_service.total_energy_cycled_kwh,
        "efc": bam_service.efc,
    }
